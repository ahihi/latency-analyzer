...
class FilePlots:
  ...
  def _plot_channel(self, i):
    ...
    return SimpleNamespace(
      ...
      wave = self.ax.plot(self.time, channel_analysis.audio, label=f"channel {channel_num}", color=color, alpha=0.75)[0]
    )
...
class App:
  def __init__(self, root, options):
    ...
    self.start_trim_var.trace_add("write", self._update_start_trim)
  ...
  def _update_start_trim(self, *args):
    self.plots.update_start_trim(self.start_trim_var.get())
    self.fig.canvas.draw_idle()

  def _update_selected_onsets(self):
